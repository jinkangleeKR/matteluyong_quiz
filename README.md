...
- 참가자는 닉네임을 입력해 대기실에 입장한 뒤, 퀴즈 시작 전까지 실시간 채팅을 사용할 수 있습니다.
- 관리자는 대기실 참가자와 채팅을 실시간으로 확인하고, 대기 중인 참가자를 내보낼 수 있습니다. 퀴즈가 시작되면 채팅 전송과 강퇴는 자동으로 꺼집니다.
- `대기 화면으로 초기화`는 새 대기실을 열며, 이전 참가자 명단·강퇴 목록·대기실 채팅을 함께 비웁니다.
- 게임 종료 후 참가자는 전체 참가자의 순위·정답 수·점수를 확인할 수 있습니다. 퀴즈 진행 중에는 다른 참가자의 답안과 순위를 읽을 수 없습니다.
- 게임이 끝나도 결과는 바로 공개되지 않습니다. 관리자가 `결과 공개 (3초 카운트)`를 누르면 모든 참가자 화면에서 3·2·1 카운트 후 점수와 전체 순위가 함께 공개됩니다.
