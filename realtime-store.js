...
function createDemoClient(notice) {
  ...
  return {
    ...
    getUser: function () {
      return { uid: playerId, isAnonymous: true };
    },
    ...
    subscribeMyAnswers: function (gameId, listener) {
      const wrapped = function (answers) {
        listener(answers[playerId] || {});
      };
  ...
  return {
    ...
    subscribeMyAnswers: function (gameId, listener) {
      ...
      return databaseModule.onValue(
        databaseModule.ref(database, "answers/" + gameId + "/" + user.uid),
        function (snapshot) {
    ...
    subscribeAllAnswers: function (gameId, listener) {
      requireAdmin();
      return databaseModule.onValue(
        databaseModule.ref(database, "answers/" + gameId),
        function (snapshot) {
          listener(snapshot.val() || {});
        }
      );
    ...
    submitAnswer: async function (answer) {
      ...
      const answerRef = databaseModule.ref(
        database,
...
export async function createQuizClient(role) {
  ...
  try {
    ...
    return createDemoClient(
      "Firebase에 연결하지 못해 데모 모드로 전환했습니다. 설정값과 Firebase 인증 상태를 확인해 주세요."
    );
  }
}
